...
export default function Present() {
  ...
  const status = problem ? "CONNECTION PAUSED" : view?.verified ? "MISSION VERIFIED" : state?.status.toUpperCase() ?? "AWAITING SIGN IN";
  const events = state?.events.filter(e => !["clock.advanced", "worker.tick"].includes(e.type)).slice(-4).reverse() ?? [];
  return <main className={styles.stage} data-testid="presentation" data-run-id={state?.id ?? ""} data-mode={state?.mode ?? ""}>
    ...
    <section className={styles.hero}>
      <div><p className={styles.eyebrow}>THE MISSION</p><h1>Trustworthy meter operations.<br /><span>Even as the attack changes.</span></h1></div>
    ...
    {!state || !view ? <div className={styles.waiting}>The recording view displays the same authenticated evidence as the dashboard.</div> : <>
      ...
      <section className={styles.domains} aria-label="Four domains">
        <article><div className={styles.domainHead}><Code2 /><h2>Code</h2><span>01</span></div><h3>{view.passed("code") ? "Trusted code serving" : "Deployment needs repair"}</h3><p>{view.workers.map(w => `${w.name}: ${w.artifactId}`).join(" · ") || "No active worker"}</p><div className={styles.evidence}>{view.patch ? <><strong>{view.patch.validation === "tested" ? "Corrective mapping tested" : "Candidate awaiting tests"}</strong><span>{view.patch.digest.slice(0, 16)}</span>{view.patch.prUrl && <a href={view.patch.prUrl} target="_blank" rel="noreferrer">Corrective pull request <ArrowUpRight size={15} /></a>}</> : <><strong>Inspect → test → promote</strong><span>Corrective artifact not yet prepared</span></>}</div></article>
        ...
        <article><div className={styles.domainHead}><MapPin /><h2>Physical assets</h2><span>04</span></div><h3>{view.fields.filter(w => w.status === "confirmed").length} / {view.fields.length} field confirmations</h3><p>Installed meters checked against their digital records</p><div className={styles.evidence}><strong>{view.passed("physical") ? "Operations acknowledgement verified" : "Awaiting trusted field evidence"}</strong><span>Simulated installations · Human confirmation</span></div></article>
      </section>
