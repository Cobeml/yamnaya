...
async function githubWithConfig(
  ...
  if (!response.ok)
    throw new Error(
      `GitHub ${response.status}: ${String(data.message ?? "request failed")}`,
    );
  return data;
}
export async function preparePatch(
  ...
  await writeFile(
    path.join(folder, "test-receipt.json"),
    JSON.stringify(result, null, 2),
  );
  const proof: EffectProof = {
  ...
  async function ensureBranch(name: string, parent: string) {
    try {
      await github(`git/ref/heads/${encodeURIComponent(name)}`);
    } catch (e) {
      if (!String(e).includes("404")) throw e;
      await github("git/refs", {
        method: "POST",
        body: JSON.stringify({ ref: `refs/heads/${name}`, sha: parent }),
      });
    }
  }
  await ensureBranch(base, sha);
  ...
  async function put(ref: string, content: string, message: string) {
    ...
    try {
      ...
      previous = String(existing.sha);
      if (
        Buffer.from(String(existing.content), "base64").toString() === content
      ) {
        const current = await github(
          `git/ref/heads/${encodeURIComponent(ref)}`,
        );
        return (current.object as { sha: string }).sha;
      }
    } catch (e) {
      if (!String(e).includes("404")) throw e;
    }
    const out = await github(`contents/${file}`, {
      method: "PUT",
      body: JSON.stringify({
        message,
  ...
  const incidentHead = await put(
    base,
    run.artifacts.find((a) => !a.trusted)!.source,
    `Seed synthetic incident ${run.id}`,
  );
  await ensureBranch(branch, incidentHead);
  proof.commit = await put(
    branch,
    source,
  ...
  if (!existingResponse.ok)
    throw new Error("Could not reconcile existing incident pull requests");
  ...
  if (existing.length) proof.externalRef = existing[0].html_url;
    ...
    const pr = await github("pulls", {
      method: "POST",
      body: JSON.stringify({
        title: `Repair meter-exchange mappings (${run.id})`,
        head: branch,
        base,
        body: `The synthetic contractor revision leaves outgoing meters active and can assign replacement meters to another service point. This change restores the source service point and ends the outgoing relationship at the exchange effective date.\n\nValidation: ${result.result.tests} isolated regression cases passed, covering unrelated relationships, target service points, end dates, and input immutability.\n\nArtifact SHA-256: ${proof.sourceDigest}\nPlan: ${plan.id} v${plan.version}. This PR targets an isolated incident branch.`,
      }),
    });
