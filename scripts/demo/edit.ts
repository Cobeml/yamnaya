...
async function main() {
  ...
  try {
    for (let i = 0; i < edit.segments.length; i++) {
      ...
      const media = s.source ? path.resolve(directory, s.source) : source;
      ...
      const label = `${edit.preview ? "CAPTURE PREVIEW" : "LIVE AGENTS - EDITED FOR TIME"} | ${manifest.mode?.toUpperCase()} | SYNTHETIC UTILITY | ${edit.runId}`;
      ...
      await command("ffmpeg", ["-hide_banner", "-loglevel", "error", "-ss", String(s.start), "-t", String(s.end - s.start), "-i", media, "-vf", filter, "-t", String(s.seconds), "-an", "-c:v", "libx264", "-preset", "fast", "-crf", "18", "-pix_fmt", "yuv420p", "-movflags", "+faststart", file]);
      clipNames.push(name);
    ...
    await writeFile(path.join(out, "verification.json"), JSON.stringify({ duration: actual, expected, width: stream.width, height: stream.height, codec: stream.codec_name, frameRate: stream.r_frame_rate, pixelFormat: stream.pix_fmt, narration: "silent", preview: edit.preview }, null, 2));
