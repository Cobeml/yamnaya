...
In the Research tab, start a workflow for an existing publication. The finder documents at least two exact retained passages with author, edition, original date, language, translation, locator and limitations. Contemporary secondary analysis is restricted to Jamestown Foundation (`jamestown.org`), Palladium Magazine (`palladiummag.com`), Jain Family Institute (`jfiresearch.org`) and Phenomenal World (`phenomenalworld.org`), including their `www` hostnames. Original texts can come from other public archives. Source classifications and historical assertions still need editorial review; a hostname check cannot establish authenticity. The worker supplies the current source policy on each invocation so older saved profiles and checkpoints do not reinstate an obsolete list.
...
Influence records distinguish independent citations, discussion, reuse and follow-up from internal references. These are operator-attested evidence links, not a fabricated readership metric. Discord sends a daily digest after 18:00 America/New_York, review/error notices, and 7-/30-day outcome-review reminders for confirmed outbound work. Delivery records prevent automatic duplicates; unknown sends remain indeterminate.
...
| `CAMP_DISCORD_BOT_TOKEN`, `CAMP_DISCORD_OPERATOR_IDS` | Discord bot token and comma-separated operator user IDs. Enable Message Content Intent and bind each camp to a distinct server channel/thread. See [Discord setup](discord.md). |

[Gmail scopes](https://developers.google.com/workspace/gmail/api/auth/scopes) and [OAuth setup](https://developers.google.com/identity/protocols/oauth2). No mailbox-reading credential, social-media password or paid search subscription is needed. Signal and MCP remain future connectors; Discord is the implemented notification path.
